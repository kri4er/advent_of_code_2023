...
fn eval_power(set: &str) -> i64 {
    ...
    let mut counter:Vec<u32> = m.into_iter().map(|(_, val)| val).collect();

    counter.sort();

    match counter.as_slice() {
        [5] => 10,
        [1, 4] => 9,
        [2, 3] => 8,
        [1, 1, 3] => 7,
...
mod test_day7 {
    ...
    fn test_card_comparison() {
        ...
        }
        {
            ...
            println!("LOGME before: {:?}", ve);
            ve.sort();
            println!("LOGME after sort: {:?}", ve);
            let mut result:i64 = 0;
            for (i, _) in ve.iter().rev().enumerate() {
                println!("Card: {:?} == {:?}", i + 1, ve[i]);
                result += (i as i64 + 1) * &ve[i].bid;
            }
            println!("LOGME End of test");
